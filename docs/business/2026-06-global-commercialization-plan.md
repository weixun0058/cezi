...
当前中文算事依赖“三个汉字 + 笔画数”。英语用户无法自然理解这个交互，因此英文版 `/ask-oracle` 建议提供两种输入：

1. 自由提问：用户输入一个清晰问题或当前困惑。
2. 三个数字：用户输入 3 个 0-999 的数字，作为无汉字门槛的起卦方式。

英文引导语建议：

`Hold one clear question in mind. Type it below, or enter three numbers that feel right to you.`

不要使用投资、疾病、法律结果作为示例。避免：

`Will my investment succeed this month?`

建议示例：

`What should I reflect on before making this decision?`

文本起卦算法原则：

1. 同一句英文问题应得到同一个签号，保证可复现。
2. 算法要简单、可解释、无外部依赖。
3. 输入为空或过短时给出友好提示，不默认返回固定签。
4. 第一版可以使用文本规范化 + 字符权重 + 哈希/求余映射到 1-384。
5. 算法要写入测试，确保同输入同结果、不同输入分布合理、边界输入不会报错。

数字起卦算法原则：

1. 三个数字范围为 0-999。
2. 计算过程要可解释，并映射到 1-384。
3. 结果页不强调“随机决定命运”，而强调“作为反思入口”。
...
5. AdSense 广告位数量和位置。
6. 英文算事首版是否同时支持自由提问和三数字输入。
7. 文本到签号算法采用字符权重、哈希，还是其他可解释映射。
