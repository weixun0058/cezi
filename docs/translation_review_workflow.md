...
| 文件 | 用途 | 修改方 |
|---|---|---|
| `data/content/oracle_signs_reinterpreted.json` | 中文解读权威源 | DeepSeek 生成 / 复检时覆盖 |
...
**注意**：数据库（reference.db）是派生物，不可作为数据源。
...
将 `reinterpreted.json` 和 `en.json` 的最新内容同步到 `reference.db`。
